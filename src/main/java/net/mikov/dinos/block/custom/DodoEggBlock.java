...
public class DodoEggBlock extends Block {
    ...
    public DodoEggBlock(Settings settings) {
        super(settings);
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && DodoEggBlock.isDirtBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    DodoEntity dodoEntity = ModEntities.DODO.create(world);
                    if (dodoEntity == null) continue;
                    dodoEntity.setBreedingAge(-24000);
                    //dodoEntity.setHomePos(pos);
                    dodoEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return DodoEggBlock.isDirt(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (DodoEggBlock.isDirtBelow(world, pos) && !world.isClient) {
