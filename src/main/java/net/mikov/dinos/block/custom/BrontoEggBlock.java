...
public class BrontoEggBlock extends AbstractLargeEggBlock {
    private static final int HATCHING_TIME = 36000;

    public BrontoEggBlock(Settings settings) {
        super(settings);
    }
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return BrontoEggBlock.isDirt(world, pos.down());
    }
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        BrontoEntity brontoEntity = ModEntities.BRONTO.create(world);
        if (brontoEntity != null) {
            Vec3d vec3d = pos.toCenterPos();
            brontoEntity.setBaby(true);
            brontoEntity.refreshPositionAndAngles(vec3d.getX(), vec3d.getY(), vec3d.getZ(), MathHelper.wrapDegrees(world.random.nextFloat() * 360.0f), 0.0f);
            world.spawnEntity(brontoEntity);
        }
    }
}
