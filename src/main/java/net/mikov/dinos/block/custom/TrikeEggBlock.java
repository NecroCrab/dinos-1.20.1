...
public class TrikeEggBlock extends Block {
    ...
    public TrikeEggBlock(Settings settings) {
        super(settings);
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.TRIKE_EGG_BLOCK)) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && TrikeEggBlock.isDirtBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    TrikeEntity trikeEntity = ModEntities.TRIKE.create(world);
                    if (trikeEntity == null) continue;
                    trikeEntity.setBreedingAge(-24000);
                    //trikeEntity.setHomePos(pos);
                    trikeEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
                    world.spawnEntity(trikeEntity);
                }
            }
        }
    }
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return TrikeEggBlock.isDirt(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (TrikeEggBlock.isDirtBelow(world, pos) && !world.isClient) {
    ...
    private boolean shouldHatchProgress(World world) {
        ...
        return world.random.nextInt(900) == 0;
    }
    ...
    private boolean breaksEgg(World world, Entity entity) {
        if (entity instanceof TrikeEntity || entity instanceof BatEntity) {
            return false;
        }
