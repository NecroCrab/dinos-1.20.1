...
public class MeganeuraEggBlock extends Block {
    ...
    public MeganeuraEggBlock(Settings settings) {
        super(settings);
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.MEGANEURA_EGG_BLOCK)) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && MeganeuraEggBlock.isDirtBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    MeganeuraEntity dragonflyEntity = ModEntities.MEGANEURA.create(world);
                    if (dragonflyEntity == null) continue;
                    dragonflyEntity.setBreedingAge(-24000);
                    //dragonflyEntity.setHomePos(pos);
                    dragonflyEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
                    world.spawnEntity(dragonflyEntity);
                }
            }
        }
    }
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return MeganeuraEggBlock.isDirt(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (MeganeuraEggBlock.isDirtBelow(world, pos) && !world.isClient) {
    ...
    private boolean breaksEgg(World world, Entity entity) {
        if (entity instanceof DimorphEntity || entity instanceof BatEntity) {
            return false;
        }
