...
public class TrilobiteEggBlock extends Block {
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.TRILOBITE_EGG_BLOCK)) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && TrilobiteEggBlock.isSandBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    TrilobiteEntity trilobiteEntity = ModEntities.TRILOBITE.create(world);
                    if (trilobiteEntity == null) continue;
                    trilobiteEntity.setBreedingAge(-24000);
                    trilobiteEntity.setHomePos(pos);
                    trilobiteEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
                    world.spawnEntity(trilobiteEntity);
                }
            }
        }
    }
    ...
    public static boolean isSandBelow(BlockView world, BlockPos pos) {
        return TrilobiteEggBlock.isSand(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (TrilobiteEggBlock.isSandBelow(world, pos) && !world.isClient) {
    ...
    private boolean breaksEgg(World world, Entity entity) {
        if (entity instanceof TrilobiteEntity || entity instanceof BatEntity) {
            return false;
        }
