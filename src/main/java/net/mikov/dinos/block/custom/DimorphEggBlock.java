...
public class DimorphEggBlock extends Block {
    ...
    public DimorphEggBlock(Settings settings) {
        super(settings);
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.DIMORPH_EGG_BLOCK)) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && DimorphEggBlock.isDirtBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    DimorphEntity dimorphEntity = ModEntities.DIMORPH.create(world);
                    if (dimorphEntity == null) continue;
                    dimorphEntity.setBreedingAge(-24000);
                    //dimorphEntity.setHomePos(pos);
                    dimorphEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
                    world.spawnEntity(dimorphEntity);
                }
            }
        }
    }
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return DimorphEggBlock.isDirt(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (DimorphEggBlock.isDirtBelow(world, pos) && !world.isClient) {
