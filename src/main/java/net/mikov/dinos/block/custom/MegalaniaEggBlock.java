...
public class MegalaniaEggBlock extends Block {
    ...
    public MegalaniaEggBlock(Settings settings) {
        super(settings);
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.MEGALANIA_EGG_BLOCK)) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && MegalaniaEggBlock.isDirtBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    MegalaniaEntity megalaniaEntity = ModEntities.MEGALANIA.create(world);
                    if (megalaniaEntity == null) continue;
                    megalaniaEntity.setBreedingAge(-24000);
                    //megalaniaEntity.setHomePos(pos);
                    megalaniaEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
                    world.spawnEntity(megalaniaEntity);
                }
            }
        }
    }
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return MegalaniaEggBlock.isDirt(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (MegalaniaEggBlock.isDirtBelow(world, pos) && !world.isClient) {
    ...
    private boolean shouldHatchProgress(World world) {
        ...
        if ((double)f < 0.75 && (double)f > 0.25) {
            return true;
        }
        return world.random.nextInt(1100) == 0;
    }
    ...
    private boolean breaksEgg(World world, Entity entity) {
        if (entity instanceof MegalaniaEntity || entity instanceof BatEntity) {
            return false;
        }
