...
public class CeratoEggBlock extends Block {
    ...
    public CeratoEggBlock(Settings settings) {
        super(settings);
    ...
    private void tryBreakEgg(World world, BlockState state, BlockPos pos, Entity entity, int inverseChance) {
        ...
        if (!world.isClient && world.random.nextInt(inverseChance) == 0 && state.isOf(ModBlocks.CERATO_EGG_BLOCK)) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (this.shouldHatchProgress(world) && CeratoEggBlock.isDirtBelow(world, pos)) {
            ...
            if (i < 2) {
                ...
                for (int j = 0; j < state.get(EGGS); ++j) {
                    world.syncWorldEvent(WorldEvents.BLOCK_BROKEN, pos, Block.getRawIdFromState(state));
                    CeratoEntity ceratoEntity = ModEntities.CERATO.create(world);
                    if (ceratoEntity == null) continue;
                    ceratoEntity.setBreedingAge(-24000);
                    //ceratoEntity.setHomePos(pos);
                    ceratoEntity.refreshPositionAndAngles((double)pos.getX() + 0.3 + (double)j * 0.2, pos.getY(), (double)pos.getZ() + 0.3, 0.0f, 0.0f);
                    world.spawnEntity(ceratoEntity);
                }
            }
        }
    }
    ...
    public static boolean isDirtBelow(BlockView world, BlockPos pos) {
        return CeratoEggBlock.isDirt(world, pos.down());
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        if (CeratoEggBlock.isDirtBelow(world, pos) && !world.isClient) {
    ...
    private boolean breaksEgg(World world, Entity entity) {
        if (entity instanceof CeratoEntity || entity instanceof BatEntity) {
            return false;
        }
