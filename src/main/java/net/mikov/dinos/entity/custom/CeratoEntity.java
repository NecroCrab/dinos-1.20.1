...
public class CeratoEntity extends AbstractDonkeyEntity
        implements InventoryChangedListener, RideableInventory, Tameable, JumpingMount, Saddleable {
    private static final TrackedData<Boolean> HAS_EGG = DataTracker.registerData(CeratoEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Boolean> DIGGING_DIRT = DataTracker.registerData(CeratoEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    int dirtDiggingCounter;
    private static final TrackedData<Byte> HORSE_FLAGS = DataTracker.registerData(CeratoEntity.class, TrackedDataHandlerRegistry.BYTE);
    private static final int TAMED_FLAG = 2;
    @Nullable
    private UUID ownerUuid;
    protected static final TrackedData<Optional<UUID>> OWNER_UUID = DataTracker.registerData(CeratoEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID);
    ...
    private static final TrackedData<Boolean> CHEST = DataTracker.registerData(CeratoEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    public CeratoEntity(EntityType<? extends AbstractDonkeyEntity> entityType, World world) {
    ...
    private static final TrackedData<Boolean> ATTACKING =
            DataTracker.registerData(CeratoEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add( 4, new CeratoAttackGoal(this, 1.15, true));
        ...
        if (!this.isTame()) {
            this.targetSelector.add(1, new RevengeGoal(this, new Class[0]).setGroupRevenge(CeratoEntity.class));
    ...
    protected void initAttributes(Random random) {
        this.getAttributeInstance(EntityAttributes.GENERIC_MAX_HEALTH).setBaseValue(CeratoEntity.getChildHealthBonus(random::nextInt));
    }
    ...
    public AbstractDonkeyEntity createChild(ServerWorld world, PassiveEntity entity) {
        return ModEntities.CERATO.create(world);
    }
    ...
    public boolean canBreedWith(AnimalEntity other) {
        ...
        if (other instanceof CeratoEntity) {
            return this.canBreed();
        }
        return false;
    }
        ...
        if (mappedIndex == 499) {
            return new StackReference(){
                ...
                public ItemStack get() {
                    return CeratoEntity.this.hasChest() ? new ItemStack(Items.CHEST) : ItemStack.EMPTY;
                }

                @Override
                public boolean set(ItemStack stack) {
                    if (stack.isEmpty()) {
                        if (CeratoEntity.this.hasChest()) {
                            CeratoEntity.this.setHasChest(false);
                            CeratoEntity.this.onChestedStatusChanged();
                        }
                        return true;
                    }
                    if (stack.isOf(Items.CHEST)) {
                        if (!CeratoEntity.this.hasChest()) {
                            CeratoEntity.this.setHasChest(true);
                            CeratoEntity.this.onChestedStatusChanged();
                        }
                        return true;
                    }
    ...
    static class MateGoal
        ...
        MateGoal(CeratoEntity cerato, double speed) {
            super(cerato, speed);
            this.cerato = cerato;
        }
        ...
        public boolean canStart() {
            return super.canStart() && !this.cerato.hasEgg();
        }
    ...
    static class LayEggGoal
        ...
        LayEggGoal(CeratoEntity cerato, double speed) {
            super(cerato, speed, 16);
            this.cerato = cerato;
        }
        ...
        public boolean shouldContinue() {
            return super.shouldContinue() && this.cerato.hasEgg();
        }

        @Override
        public void tick() {
            super.tick();
            BlockPos blockPos = this.cerato.getBlockPos();
            if (!this.cerato.isTouchingWater() && this.hasReached()) {
                if (this.cerato.dirtDiggingCounter < 1) {
                    this.cerato.setDiggingDirt(true);
                } else if (this.cerato.dirtDiggingCounter > this.getTickCount(200)) {
                    World world = this.cerato.getWorld();
                    ...
                    BlockState blockState = ModBlocks.CERATO_EGG_BLOCK.getDefaultState().with(CeratoEggBlock.EGGS, this.cerato.random.nextInt(4) + 1);
                    world.setBlockState(blockPos2, blockState, Block.NOTIFY_ALL);
                    world.emitGameEvent(GameEvent.BLOCK_PLACE, blockPos2, GameEvent.Emitter.of(this.cerato, blockState));
                    this.cerato.setHasEgg(false);
                    this.cerato.setDiggingDirt(false);
                    this.cerato.setLoveTicks(600);
                }
        ...
        protected boolean isTargetPos(WorldView world, BlockPos pos) {
            ...
            return CeratoEggBlock.isDirt(world, pos);
        }
    }
    ...
    public boolean canAttackWithOwner(LivingEntity target, LivingEntity owner) {
        ...
        if (target instanceof CeratoEntity) {
            CeratoEntity ceratoEntity = (CeratoEntity)target;
            return !ceratoEntity.isTame() || ceratoEntity.getOwner() != owner;
        }
    ...
    public static class TrackOwnerAttackerGoal
        ...
        public TrackOwnerAttackerGoal(CeratoEntity cerato) {
            super(cerato, false);
            this.cerato = cerato;
        ...
        @Override
        public boolean canStart() {
            if (!this.cerato.isTame()) {
                return false;
            }
            LivingEntity livingEntity = this.cerato.getOwner();
            if (livingEntity == null) {
                return false;
            }
            this.attacker = livingEntity.getAttacker();
            int i = livingEntity.getLastAttackedTime();
            return i != this.lastAttackedTime && this.canTrack(this.attacker, TargetPredicate.DEFAULT) && this.cerato.canAttackWithOwner(this.attacker, livingEntity);
        }
    ...
    public static class AttackWithOwnerGoal
        ...
        public AttackWithOwnerGoal(CeratoEntity cerato) {
            super(cerato, false);
            this.cerato = cerato;
            this.setControls(EnumSet.of(Control.TARGET));
        }
        ...
        public boolean canStart() {
            ...
            return i != this.lastAttackTime && this.canTrack(this.attacking, TargetPredicate.DEFAULT) && this.cerato.canAttackWithOwner(this.attacking, livingEntity);
        }
