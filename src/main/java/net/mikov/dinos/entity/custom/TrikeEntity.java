...
import net.mikov.dinos.block.custom.TrikeEggBlock;
import net.mikov.dinos.entity.ModEntities;
import net.mikov.dinos.entity.ai.AnkyAttackGoal;
import net.mikov.dinos.entity.ai.TrikeAttackGoal;
...
public class TrikeEntity extends AbstractDonkeyEntity
        implements InventoryChangedListener, RideableInventory, Tameable, JumpingMount, Saddleable {
    private static final TrackedData<Boolean> HAS_EGG = DataTracker.registerData(TrikeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Boolean> DIGGING_DIRT = DataTracker.registerData(TrikeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    int dirtDiggingCounter;
    private static final TrackedData<Byte> HORSE_FLAGS = DataTracker.registerData(TrikeEntity.class, TrackedDataHandlerRegistry.BYTE);
    private static final int TAMED_FLAG = 2;
    @Nullable
    private UUID ownerUuid;
    protected static final TrackedData<Optional<UUID>> OWNER_UUID = DataTracker.registerData(TrikeEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID);
    ...
    private static final TrackedData<Boolean> CHEST = DataTracker.registerData(TrikeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    public static final int field_30412 = 15;

    public TrikeEntity(EntityType<? extends AbstractDonkeyEntity> entityType, World world) {
    ...
    private static final TrackedData<Boolean> ATTACKING =
            DataTracker.registerData(TrikeEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(4, new TrikeAttackGoal(this, 1.3, true));
        ...
        if (!this.isTame()) {
            this.targetSelector.add(3, new RevengeGoal(this, new Class[0]).setGroupRevenge(TrikeEntity.class));
    ...
    public static DefaultAttributeContainer.Builder createTrikeAttributes() {
    ...
    protected void initAttributes(Random random) {
        this.getAttributeInstance(EntityAttributes.GENERIC_MAX_HEALTH).setBaseValue(TrikeEntity.getChildHealthBonus(random::nextInt));
    }
    ...
    public PassiveEntity createChild(ServerWorld world, PassiveEntity entity) {
        return ModEntities.TRIKE.create(world);
        //return null;
    }
    ...
    public boolean canBreedWith(AnimalEntity other) {
        ...
        if (other instanceof TrikeEntity) {
            return this.canBreed();
        }
        return false;
    }
        ...
        if (mappedIndex == 499) {
            return new StackReference(){
                ...
                public ItemStack get() {
                    return TrikeEntity.this.hasChest() ? new ItemStack(Items.CHEST) : ItemStack.EMPTY;
                }

                @Override
                public boolean set(ItemStack stack) {
                    if (stack.isEmpty()) {
                        if (TrikeEntity.this.hasChest()) {
                            TrikeEntity.this.setHasChest(false);
                            TrikeEntity.this.onChestedStatusChanged();
                        }
                        return true;
                    }
                    if (stack.isOf(Items.CHEST)) {
                        if (!TrikeEntity.this.hasChest()) {
                            TrikeEntity.this.setHasChest(true);
                            TrikeEntity.this.onChestedStatusChanged();
                        }
                        return true;
                    }
    ...
    static class MateGoal
        ...
        private final TrikeEntity trike;

        MateGoal(TrikeEntity trike, double speed) {
            super(trike, speed);
            this.trike = trike;
        }
        ...
        public boolean canStart() {
            return super.canStart() && !this.trike.hasEgg();
        }
    ...
    static class LayEggGoal
        ...
        LayEggGoal(TrikeEntity trike, double speed) {
            super(trike, speed, 16);
            this.trike = trike;
        }

        @Override
        public boolean canStart() {
            if (this.trike.hasEgg()) {
        ...
        public boolean shouldContinue() {
            return super.shouldContinue() && this.trike.hasEgg();
        }

        @Override
        public void tick() {
            super.tick();
            BlockPos blockPos = this.trike.getBlockPos();
            if (!this.trike.isTouchingWater() && this.hasReached()) {
                if (this.trike.dirtDiggingCounter < 1) {
                    this.trike.setDiggingDirt(true);
                } else if (this.trike.dirtDiggingCounter > this.getTickCount(200)) {
                    World world = this.trike.getWorld();
                    ...
                    BlockState blockState = ModBlocks.TRIKE_EGG_BLOCK.getDefaultState().with(TrikeEggBlock.EGGS, this.trike.random.nextInt(4) + 1);
                    world.setBlockState(blockPos2, blockState, Block.NOTIFY_ALL);
                    world.emitGameEvent(GameEvent.BLOCK_PLACE, blockPos2, GameEvent.Emitter.of(this.trike, blockState));
                    this.trike.setHasEgg(false);
                    this.trike.setDiggingDirt(false);
                    this.trike.setLoveTicks(600);
                }
                if (this.trike.isDiggingDirt()) {
                    ++this.trike.dirtDiggingCounter;
                }
            }
        }
        ...
        protected boolean isTargetPos(WorldView world, BlockPos pos) {
            ...
            return TrikeEggBlock.isDirt(world, pos);
        }
    }
    ...
    public boolean canAttackWithOwner(LivingEntity target, LivingEntity owner) {
        ...
        if (target instanceof TrikeEntity) {
            TrikeEntity trikeEntity = (TrikeEntity)target;
            return !trikeEntity.isTame() || trikeEntity.getOwner() != owner;
        }
    ...
    public static class TrackOwnerAttackerGoal
        ...
        public TrackOwnerAttackerGoal(TrikeEntity trike) {
            super(trike, false);
            this.trike = trike;
        ...
        @Override
        public boolean canStart() {
            if (!this.trike.isTame()) {
                return false;
            }
            LivingEntity livingEntity = this.trike.getOwner();
            if (livingEntity == null) {
                return false;
            }
            this.attacker = livingEntity.getAttacker();
            int i = livingEntity.getLastAttackedTime();
            return i != this.lastAttackedTime && this.canTrack(this.attacker, TargetPredicate.DEFAULT) && this.trike.canAttackWithOwner(this.attacker, livingEntity);
        }
    ...
    public static class AttackWithOwnerGoal
        ...
        public AttackWithOwnerGoal(TrikeEntity trike) {
            super(trike, false);
            this.trike = trike;
            this.setControls(EnumSet.of(Control.TARGET));
        }
        ...
        public boolean canStart() {
            ...
            return i != this.lastAttackTime && this.canTrack(this.attacking, TargetPredicate.DEFAULT) && this.trike.canAttackWithOwner(this.attacking, livingEntity);
        }
