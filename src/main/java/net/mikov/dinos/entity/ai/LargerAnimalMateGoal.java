...
public class LargerAnimalMateGoal
        extends Goal {
    ...
    public LargerAnimalMateGoal(AnimalEntity animal, double speed) {
        this(animal, speed, animal.getClass());
    }

    public LargerAnimalMateGoal(AnimalEntity animal, double speed, Class<? extends AnimalEntity> entityClass) {
        this.animal = animal;
    ...
    public void tick() {
        ...
        ++this.timer;
        if (this.timer >= this.getTickCount(60) && this.animal.squaredDistanceTo(this.mate) < 26.0) {
            this.breed();
        }
    }
